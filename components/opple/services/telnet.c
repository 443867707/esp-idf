...
void aps_telnet_init(void)
{
	MUTEX_CREATE(mutex_telnet);
}
...
int telnet_esp32_vprintf(const char *fmt, va_list va) {
	...
	MUTEX_LOCK(mutex_telnet, MUTEX_WAIT_ALWAYS);
	telnet_vprintf(tnHandle, fmt, va);
	MUTEX_UNLOCK(mutex_telnet);
	//printf("telnet_esp32_vprintf done\r\n");
	return 0;
...
static void doTelnet(int partnerSocket) {	
  ...
  setsockopt(partnerSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(struct timeval));
